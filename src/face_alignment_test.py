...
if __name__ == '__main__':
	#delete file in test repo
	listfile = os.listdir('../data/test_112x112')
	if len(listfile) > 0:
		os.system('rm ../data/test_112x112/*')
	listfile = os.listdir('../data/test_160x160')
	if len(listfile) > 0:
		os.system('rm ../data/test_160x160/*')
	listfile = os.listdir('../data/test_unknown')
	if len(listfile) > 0:
		os.system('rm ../data/test_unknown/*')
	...
	#test = pd.DataFrame(columns = ["image", "class"])
	
	for i in range(labels.shape[0]):
		print("Image : {} ----- File : {} ------\n".format(i+1, labels['image'][i]))
		img = io.imread('../data/test/' + labels['image'][i])
		...
		if len(results) != 0:
			...
			if landmarks != None:
				...
				io.imsave('../data/test_112x112/'+labels['image'][i], new_face_112x112)
				io.imsave('../data/test_160x160/'+labels['image'][i], new_face_160x160)
				#train.append(labels[i])
			else:
				print("Cannot detect !")
				io.imsave('../data/test_unknown/'+labels['image'][i], img)
		else:
			print("Cannot detect !")
			io.imsave('../data/test_unknown/'+labels['image'][i], img)
	#train.to_csv("../data/train_embedding.csv")
	labels.to_csv("../data/test.csv")

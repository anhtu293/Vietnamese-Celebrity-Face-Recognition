...
parser.add_argument("--embeddings", default = "./embeddings/facenet/embs_class_train_160x160_20180402-114759.npy", help = "path to file index embeddings for train")
...
def DataGenerator(labels, classes, batch_size):
    ...
    while True:
        ...
        for b in range(batch_size):
            if i == labels.shape[0]:
                i = 0
            sample = np.load(labels[0][i])
            cl = classes[i]
            embeddings_batch.append(sample)
...
if __name__ == '__main__':
    ...
    callbacks = [
		...
		tf.keras.callbacks.ModelCheckpoint(filepath ="./checkpoints/"+args.model+"/weights-epoch{epoch:02d}-loss{val_loss:.2f}.h5")
		]
    history = model.fit_generator(generator = train_generator, epochs = 100, verbose = 1,
        validation_data = val_generator, steps_per_epoch = len(labels_train)/BATCH_SIZE,
        validation_steps = len(labels_val)/BATCH_SIZE)
    
    model.save("./model/facenet/" + args.model + "/model_" + args.model + "_" + args.size + ".h5")